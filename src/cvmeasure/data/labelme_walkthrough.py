...
def write_labelme_example(raw_dir: Path, seed: int = 17) -> None:
    ...
    for animal_id in SPLITS:
        ...
        annotation = {
            ...
            "description": "Synthetic cvmeasure frame. This is a reproducibility fixture, not a real livestock annotation.",
        }
...
def write_overlay(image_path: Path, mask_path: Path, output_path: Path) -> None:
    ...
    for class_id, colour in {1: (50, 210, 50), 2: (40, 170, 255)}.items():
        contours, _ = cv2.findContours((mask == class_id).astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def build_walkthrough(output_root: Path, seed: int = 17) -> None:
    raw_dir, dataset_dir = output_root / "labelme_raw", output_root / "imported_dataset"
    write_labelme_example(raw_dir, seed=seed)
    metadata = import_labelme(
        raw_dir, dataset_dir, pixel_spacing_mm=0.42, split_fn=lambda frame_id: SPLITS[frame_id.split("_")[0]]
    )
    ...
    columns = ["id", "animal_id", "split", "pixel_spacing_mm", "ema_mm2", "emd_mm", "emw_mm", "fat_c_mm"]
